...
def search_notes(notes, keywords=None, status=None):
    ...
    print(Fore.CYAN + "\n[INFO]: Вы можете искать по нескольким ключевым словам (разделяйте их пробелом).\n" +
          "Пример статусов: 'новая', 'в процессе', 'отложено', 'выполнено'." + Style.RESET_ALL)
...
def main():
    ...
    while True:
        ...
        if choice == 1:
            ...
            status = input("Введите статус для поиска (например, 'новая', 'в процессе', Enter для пропуска): ").strip()
...
if __name__ == "__main__":
    ...
    #Меню еще прежде реализовал, тут объединил две кнопки удаления в "Удалить заметки"