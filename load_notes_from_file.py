...
def load_notes_from_file(filename):
    """
    Загружает заметки из текстового файла и преобразует их в список словарей.
    """

    if not os.path.exists(filename):
        print(f"\n[INFO]:\033[31m Файл '{filename}' не найден. Возвращён пустой список. \033[0m")
        return []

    notes = []
    try:
        with open(filename, 'r', encoding='utf-8') as file:
            # Читаем содержимое файла построчно и обрабатываем заметки
            lines = file.readlines()
            note = {}

            if not lines:
                print(f"\n[INFO]:\033[31m Файл '{filename}' пуст. Возвращается пустой список заметок.\033[0m")
                return []

            for line in lines:
                line = line.strip()  # Убираем лишние пробелы и символы новой строки
                if line == "---":  # Указатель конца одной заметки
                    if note:       # Сохраняем собранную заметку
                        notes.append(note)
                        note = {}
                elif line:  # Если строка не пустая, обрабатываем её
                    key, value = line.split(":", 1)
                    key = key.strip()
                    value = value.strip()
                    if key == "Заголовок":
                        note[key] = value.split(", ")  # Преобразуем в список, если это заголовок
                    else:
                        note[key] = value
            if note:  # Добавляем последнюю заметку, если она не была завершена "---"
                notes.append(note)
        return notes
    except Exception as e:
        print(f"\n[ERROR]:\033[31m Ошибка при загрузке заметок из файла '{filename}'. Ошибка: {e} \033[0m")
        return []
...
def save_notes_to_file(notes, filename):
    ...
    try:
        ...
        print(f"\n {Fore.GREEN}[INFO]: Заметки успешно сохранены в файл '{filename}'!{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}[ERROR]: Не удалось сохранить заметки в файл. Ошибка: {e}{Style.RESET_ALL}")


def handle_save_or_load(notes, filename):
    """
    Обрабатывает выбор между сохранением и загрузкой заметок.

    """
    print("\nВыберите действие с заметками:")
    print("1. Сохранить заметки в файл")
    print("2. Загрузить заметки из файла")
    ...
    if choice == 1:
        ...
        if loaded_notes:
            notes.clear()  # Очистка текущего списка заметок
            notes.extend(loaded_notes)  # Добавляем загруженные заметки
            print(f"\n[INFO]:\033[92m Заметки успешно загружены из файла '{filename}'.\033[0m")
    else:
...
def main():
    ...
    while True:
        ...
        print("6. Сохранить или Загрузить заметки")
        print("7. Выйти")
        ...
        if choice == 1:
            ...
            handle_save_or_load(notes, filename)
        elif choice == 7:
